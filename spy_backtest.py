...
def inspect_and_clean(raw_prices):
    ...
    if "Close" not in prices.column():
        raise KeyError("Expected Close column but no column present")
...
def averages_and_signal(prices):
    ...
    # Use the previous trading day's signal to avoid look-ahead bias
    prices["Position"] = prices["Signal"].shift(1)
    
    return prices


def select_test_period(prices, start, end):
    
    """
    Removes the warmup data 
    """
    
    prices = prices.loc[start:end].copy()
    
    if prices.empty:
        raise RuntimeError("No data exists in this period")
...
def strat_return_and_growth(prices, transaction_cost = 0.0005):
    ...
    prices["Trade"] = prices["Position"].diff()
    
    first_date = prices.index[0]
    prices.loc[first_date, "Trade"] = prices.loc[first_date, "Position"]
    
    prices["Trade"] = prices["Trade"].fillna(0)
    
    prices.loc[first_date, "Daily Return"] = 0.0
...
def metrics(prices):
    ...
    print("Strategy Volatility:", strategy_vol)
    ...
    print("\nEntries:", entries)
    print("Exits:", exits)
    print("Starting position:", prices["Position"].iloc[0])
    print("End position:", prices["Position"].iloc[-1])
    ...
    for i in range(len(prices)):
        ...
        strategy_return = prices["Strategy Return"].iloc[i]

        # Begin tracking a new trade
        if trade == 1:
            trade_growth = 1.0

        # Compound every net strategy return while the trade is open
        if trade_growth is not None:
            trade_growth *= 1 + strategy_return

        # Record the completed net return after the exit cost
        if trade == -1 and trade_growth is not None:
            trade_returns.append(trade_growth - 1)
            trade_growth = None

    trade_returns = pd.Series(trade_returns, dtype=float)

    open_trade = trade_growth is not None
   
   
  
    print("\nCompleted Trades:", len(trade_returns))
    print("Open Trade at End:", open_trade)

    if not trade_returns.empty:
        print("Average Net Trade Return:", trade_returns.mean() * 100, "%")
        print("Highest Net Trade Return:", trade_returns.max() * 100, "%")
        print("Lowest Net Trade Return:", trade_returns.min() * 100, "%")
        print(
            "Winning Trade Rate:",
...
def graphs(prices):
    ...
    #drawdowns
    prices[["Strategy Drawdown", "Market Drawdown"]
           ].plot(figsize = (10,5))
    plt.title("Market Drawdown vs Strategy Drawdown")
    plt.xlabel("Date (Yr)")
    plt.ylabel("Drawdown ")
    plt.tight_layout()
    plt.show()
...
def main():
    ...
    warmup_start = "2019-08-01"
    test_start = "2020-01-01"
    test_end = "2026-07-21"
    download_end = "2026-07-22"
    
    raw_prices = load_data("SPY", warmup_start, download_end)
    prices = inspect_and_clean(raw_prices)
    prices = daily_return(prices)
    prices = averages_and_signal(prices)
    prices = select_test_period(prices, test_start, test_end)
